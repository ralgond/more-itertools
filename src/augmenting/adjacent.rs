...
pub fn adjacent<T>(i: Box<dyn Iterator<Item=T>>, pred: fn(T)->bool, distance: usize) -> Box<dyn Iterator<Item=(bool, T)>>
where
T: Clone + 'static
{
    let (i1, i2) = tee(i.into_iter());
    let left_padding = iter_from_vec(false_padding(distance));
    let right_padding = iter_from_vec(false_padding(distance));
