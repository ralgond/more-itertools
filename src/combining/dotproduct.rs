...
macro_rules! dotproduct {
    ($type0:ty, $iter0:expr, $iter1:expr, $init:expr) => {
        {
            let mut map_iter = map2_result($iter0, $iter1, |x, y| {
...
mod tests {
    ...
    #[test]
    fn test1() {
        let iter_0 = iter_from_vec(vec![10i64,10]);
        let iter_1 = iter_from_vec(vec![20i64,20]);
        let ret = dotproduct!(i64, iter_0, iter_1, 0i64);
        assert_eq!(Ok(400i64), ret);


        let iter_0 = iter_from_vec(vec![10i64,10]);
        let iter_1 = iter_from_vec(vec![20i64,i64::MAX]);
        let ret = dotproduct!(i64, iter_0, iter_1, 0i64);
