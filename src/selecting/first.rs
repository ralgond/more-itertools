...
pub fn first<T>(iter: &mut Box<dyn Iterator<Item = Result<T,Error>>>, default: Option<T>) -> Option<Result<T, Error>>
where
T: 'static + Clone
{
    let _next = iter.next();
    if let Some(v_next) = _next {
        match v_next {
            Ok(ok_v_next) => {
                return Some(Ok(ok_v_next.clone()));
            },
            Err(err_v_next) => {
        ...
        if let Some(v_default) = default {
            ...
            return Some(Err(error::value_error(String::from("[first:empty iterable, and no default value]"))));
        }
    }
}
...
mod tests {
    ...
    fn test1() {
        ...
        assert_eq!(error::Kind::ValueError, ret3.unwrap().err().unwrap().kind());

        let v = vec![1,2,3];
        let ret4 = first(&mut generate_okok_iterator(v), Some(0));
        assert_eq!(1, ret4.unwrap().ok().unwrap());

        let ret5 = first(&mut generate_okokerr_iterator(vec![], error::overflow_error("for test".to_string())), Some(5));
        assert_eq!(error::Kind::OverflowError, ret5.unwrap().err().unwrap().kind());
    }
}
